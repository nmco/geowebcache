...
public final class MbtilesBlobStore extends SqliteBlobStore {
    ...
    // pattern for matching the name of a file that contains mbtiles metadata (layerName.metadata)
    private final static Pattern MBTILES_METADATA_FILE_NAME_PATTERN = Pattern.compile("(.*?)\\.metadata");
    ...
    public MbtilesBlobStore(MbtilesConfiguration configuration, SqliteConnectionManager connectionManager) {
        ...
        Utils.info(LOGGER, "MBTiles blob store initiated: [eagerDelete='%b', useCreateTime='%b'.", eagerDelete, useCreateTime);
    }

    @Override
    public void put(final TileObject tile) throws StorageException {
        Utils.updateLogId("PUT-" + tile.toString());
        final File file = fileManager.getFile(tile);
        Utils.debug(LOGGER, "Tile mapped to file '%s'.", file);
        initDatabaseFileIfNeeded(file, tile.getLayerName(), tile.getBlobFormat());
        // do work in write mode
        connectionManager.doWork(file, false, new SqliteConnectionManager.Work() {
            @Override
            public void doWork(Connection connection) {
                ...
                try {
                    ...
                    Utils.debug(LOGGER, "Tile saved in file '%s'.", file);
                    if (listeners.isEmpty()) {
    ...
    public boolean get(final TileObject tile) throws StorageException {
        Utils.updateLogId("GET-" + tile.toString());
        final File file = fileManager.getFile(tile);
        Utils.debug(LOGGER, "Tile mapped to file '%s'.", file);
        initDatabaseFileIfNeeded(file, tile.getLayerName(), tile.getBlobFormat());
        // do work in readonly mode
        boolean exists = connectionManager.doWork(file, true, new SqliteConnectionManager.WorkWithResult<Boolean>() {
            @Override
            public Boolean doWork(Connection connection) {
                ...
                try {
                    ...
                    if (gtTile.getData() != null) {
                        ...
                        Utils.debug(LOGGER, "Tile found on file '%s'.", file);
                        return true;
                    }
                } catch (Exception exception) {
                    throw Utils.exception(exception, "Error loading tile '%s' from MBTiles file '%s'.", tile, file);
                }
                Utils.debug(LOGGER, "Tile not found on file '%s'.", file);
                return false;
            }
        });
        if (exists && useCreateTime) {
            ...
            if (createdTime == null) {
                // no create time associated with this tile lets assume the current as the create time
    ...
    @Override
    public boolean delete(final TileObject tile) throws StorageException {
        Utils.updateLogId("DELETE-" + tile.toString());
        final File file = fileManager.getFile(tile);
        Utils.debug(LOGGER, "Tile mapped to file '%s'.", file);
        initDatabaseFileIfNeeded(file, tile.getLayerName(), tile.getBlobFormat());
        // do work on write mode
        return connectionManager.doWork(file, false, new SqliteConnectionManager.WorkWithResult<Boolean>() {
            @Override
            public Boolean doWork(Connection connection) {
                ...
                try {
                    ...
                    if (olData != null) {
                        ...
                        if (useCreateTime) {
                            ...
                            MbtilesBlobStore.this.deleteTileCreateTime(connection, tile.getXYZ()[2], tile.getXYZ()[0], tile.getXYZ()[1]);
                        }
                        Utils.debug(LOGGER, "Tile deleted from file '%s'.", file);
                        return true;
                    }
                } catch (Exception exception) {
    ...
    public synchronized void putLayerMetadata(String layerName, String key, String value) {
        ...
        Utils.updateLogId(String.format("PUT-METADATA-[layerName='%s', key='%s', value='%s']", layerName, key, value));
        ...
        Utils.info(LOGGER, "Metadata inserted or updated on file '%s'.", metadataFile);
    }

    @Override
    public String getLayerMetadata(String layerName, String key) {
        Utils.updateLogId(String.format("GET-METADATA-[layerName='%s', key='%s']", layerName, key));
        try {
            return connectionManager.executeQuery(metadataFile, new SqliteConnectionManager.ResultExtractor<String>() {
                        @Override
                        public String extract(ResultSet resultSet) throws SQLException {
                            try {
                                if (resultSet.next()) {
                                    ...
                                    Utils.info(LOGGER, "Metadata found '%s'.", value);
                                    return value;
                                }
                                // metadata value not found
                                Utils.debug(LOGGER, "Metadata not found.");
                                return null;
                            } catch (Exception exception) {
            ...
            // maybe the metadata table doesn't exists
            Utils.error(LOGGER, exception, "Error getting metadata from file '%s'.", metadataFile);
            return null;
        }
    }
    ...
    public boolean delete(String layerName) throws StorageException {
        Utils.updateLogId(String.format("DELETE-LAYER-[layerName='%s']", layerName));
    ...
    public boolean deleteByGridsetId(String layerName, String gridSetId) throws StorageException {
        Utils.updateLogId(String.format("DELETE-LAYER-GRID-SET-[layerName='%s', gridSet='%s']", layerName, gridSetId));
    ...
    public boolean delete(TileRange tileRange) throws StorageException {
        Utils.updateLogId(String.format("DELETE-TILE-RANGE-[layerName='%s']", tileRange.getLayerName()));
        ...
        if (files.isEmpty()) {
            ...
            Utils.debug(LOGGER, "Nothing to do.");
            return false;
        }
    ...
    private boolean deleteFiles(List<File> files) throws StorageException {
        if (files.isEmpty()) {
            Utils.info(LOGGER, "No files to delete.");
            return false;
        }
        ...
        Utils.debug(LOGGER, "Files deleted.");
        return true;
    }
    ...
    private void deleteTileCreateTime(Connection connection, long z, long x, long y) throws StorageException {
        try {
            ...
            Utils.error(LOGGER, exception, "Something bad happen when deleting create time for tile '%d-%d-%d'.", x, y, z);
        }
    }
    ...
    private Long getTileCreateTime(File file, long z, long x, long y) throws StorageException {
        ...
        try {
            return connectionManager.executeQuery(file, new SqliteConnectionManager.ResultExtractor<Long>() {
                @Override
            ...
            // probably the table doesn't exists
            Utils.error(LOGGER, exception, "Something bad happen when querying create time for tile '%d-%d-%d'.", x, y, z);
        }
        return null;
    }
     ...
     */
    private void putTileCreateTime(File file, final long z, final long x, final long y, final long createTime) {
        connectionManager.doWork(file, false, new SqliteConnectionManager.Work() {
            @Override
            public void doWork(Connection connection) {
     ...
     */
    void initDatabaseFileIfNeeded(final File file, final String layerName, final String format) {
        if (file.exists()) {
        ...
        connectionManager.doWork(file, false, new SqliteConnectionManager.Work() {
            ...
            public void doWork(Connection connection) {
                try {
                    ...
                    SqlUtil.runScript(MbtilesBlobStore.this.getClass().getResourceAsStream("/org/geotools/mbtiles/mbtiles.sql"), connection);
                    // create tiles metadata table for storing the create time if needed
                    MbtilesBlobStore.this.createTilesMetadataTable(connection);
                    // insert mbtiles metadata for this layer
                    MbtilesBlobStore.this.insertMbtilesLayerMetadata(file, connection, layerName, format);
                } catch (Exception exception) {
    ...
    private void initMbtilesLayersMetadata(String mbtilesMetadataDirectoryPath) {
        if (mbtilesMetadataDirectoryPath == null) {
            Utils.info(LOGGER, "Mbtiles metadata directory path is NULL, no mbtiles metadata will be parsed.");
            return;
        }
        File mbtilesMetadataDirectory = new File(mbtilesMetadataDirectoryPath);
        if (!mbtilesMetadataDirectory.exists()) {
            Utils.info(LOGGER, "Mbtiles metadata directory '%s' doesn't exists, no mbtiles metadata will be parsed.",
                    mbtilesMetadataDirectoryPath);
            return;
        }
        File[] files = mbtilesMetadataDirectory.listFiles();
        if (files == null) {
            Utils.info(LOGGER, "No files present in mbtiles metadata directory '%s', no mbtiles metadata will be parsed.",
                    mbtilesMetadataDirectoryPath);
            return;
        }
        for (File file : files) {
            ...
            if (matcher.matches()) {
                ...
                Utils.info(LOGGER, "Parsed mbtiles metadata for layer '%s'.", layerName);
            }
        }
    }
}
