...
abstract class SqliteBlobStore implements BlobStore {
    ...
    void replace(File newFile, String destination) {
        Utils.updateLogId(String.format("REPLACE-FILE-[file='%s', destination='%s']", newFile.getPath(), destination));
    ...
    void replace(File directory) {
        Utils.updateLogId(String.format("REPLACE-FILES-[directory='%s']", directory.getPath()));
